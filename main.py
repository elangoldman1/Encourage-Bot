...
async def on_message(message):
  ...
  if msg.startswith("$del"):
    encouragements = []
    if "encouragements" in db.keys():
        index_str = msg.split("$del", 1)[1].strip()
        try:
            index = int(index_str)
            if 0 <= index < len(db["encouragements"]):
                ...
                await message.channel.send("Invalid index. Please provide a valid index to delete an encouragement.")
        except ValueError:
            await message.channel.send("Invalid index format. Please provide a valid index to delete an encouragement.")
    else:
